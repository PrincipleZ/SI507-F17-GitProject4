# A comment made by Yuanze Zhang
...
# Visit the Keys and Access Tokens tab for your app and grab the following
# two values
...
def get_tokens():
    ...
    # after this line executes, oauth will now be an instance of the class
    # OAuth1Session
    ...
    # their protected resources (images, tweets, etc.). This is commonly done
    # by redirecting the user to a specific url to which you add the request
    # token as a query parameter. Note that not all services will give you a
    # verifier even if they should. Also the oauth_token given here will be
    # the same as the one in the previous step.
    ...
    # Once you log in, and give permissions, you should see a string of
    # characters that you need to copy.
    ...
    # make a new instance of the OAuth1Session class, with several more
...
try:
    ...
    (client_key, client_secret, resource_owner_key,
     resource_owner_secret, verifier) = json.loads(f.read())
    f.close()
except:
    ...
    f.close()
    (client_key, client_secret, resource_owner_key,
     resource_owner_secret, verifier) = tokens
...
# Create an instance of OAuth1Session in a variable that you can use to
# make a request
oauth = requests_oauthlib.OAuth1Session(
    client_key,
    client_secret=client_secret,
...
# The above you need to keep around to use. The rest is up to you -- what
# requests you want to make, what you do with the data you get back, etc!
...
# Same as getting .text attribute, then json.loads() on the value of the
# .text attribute, as you've seen before!


# Make a request to the Tweet search endpoint, searching for the phrase
# 'University of Michigan', looking to get 3 Tweets back
...
# Twitter actually lets you get more than 25 at a time, but not more than
# 200, so this is practice for when you want to get more than 200. You
# could get the last 1000, 200 at a time, using the same pattern.
...
# where I'll collect all the tweet data I get as I page through my
# results, 5 at a time
collected_tweets = []
ids = []
max_id = None
...
for i in range(5):
    ...
    # Regardless, now we need to make a request to the logged in user's
    # timeline with the query parameters
    # passes {'count': 5, 'max_id': whatever} ...
    r = oauth.get(
        "https://api.twitter.com/1.1/statuses/user_timeline.json",
        params=my_params)
    # Now, append this data to a list so we can collect all the paged results
    collected_tweets.append(r.json())
    # get the ids from the tweets we just got
    next_five_ids = [tweet['id'] for tweet in r.json()]
    # add them to the list, and start the for loop process over again
...
# If you're testing with the data in the file only, you may want to
# comment out all the code above this for a while so you don't
# inadvertently make a lot of requests to Twitter and then run out of
# request privileges for the day!
